...
if __name__ == "__main__":
  ...
  window = args.window # Load the experiment settings
  d = args.d 
  eta = args.eta

  alphabet, langs, lengths = count([train, dev, test]) # Count alphabet and languages
  print("--- Characters Found: {}".format(len(alphabet)))
  print(alphabet)
  ...
  for line in train:
    ...
    sections.append([sentence[start:start+window] for start in range(len(sentence)-(window-1))]) # Carve sentence into sections
  ...
  if os.path.exists('model_{}_{}.p'.format(d,eta)):
    ...
    model.add(Encoder(window, alphabet))
    model.add(Linear(window*len(alphabet), d))
    model.add(Sigmoid())
  ...
  for epoch in range(epochs):
    ...
    for i in range(samples): # Want to do as many SGD samples as sentences*their windows
      ...
      if i > 0 and i % 10000 == 0:
        loss_avg = loss_avg / 10000 
        accuracy = correct / (1.0 * tested)
        print("%.4f %4d / %4d Chunks. Accuracy: %.4f Loss: %.4f Weights: %.8f" % (i / (1.0 * samples), i, samples, accuracy, loss_avg, sum(sum(model.layers[1].weights))))
        loss_avg = 0
      else:
        loss_avg += loss
